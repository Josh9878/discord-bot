...
// Check if TikTok stream is live (using web scraping as TikTok has no public API)
    ...
    return new Promise((resolve) => {
        ...
        const req = https.request(options, (res) => {
            ...
            res.on('end', () => {
                ...
                // This is a simple check - TikTok's HTML structure may change
                const isLive = body.includes('"LiveRoom"') || body.includes('live_room');
                resolve(isLive ? { username: username, live: true } : null);
            });
        });
...
async function monitorStreams() {
    ...
    // Check TikTok
    try {
        const tiktokStream = await checkTiktokLive();
        
        if (tiktokStream && !streamData.tiktokLive) {
            // Just went live!
            streamData.tiktokLive = true;
            saveStreamData();
            await sendTiktokNotification(guild);
        } else if (!tiktokStream && streamData.tiktokLive) {
            // Stream ended
            streamData.tiktokLive = false;
            saveStreamData();
...
client.on(Events.MessageCreate, async message => {
    ...
    if (message.content.toLowerCase() === '!stream status') {
        const statusEmbed = new EmbedBuilder()
            ...
            .addFields(
                { name: 'Twitch', value: streamData.twitchLive ? '🔴 LIVE' : '⚫ Offline', inline: true },
                { name: 'TikTok', value: streamData.tiktokLive ? '🔴 LIVE' : '⚫ Offline', inline: true }
            )
